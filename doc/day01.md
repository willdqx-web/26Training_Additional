...
> **作業前の確認（必須）**
> タスクバー（画面右下）にクジラのアイコンがあり、アニメーションが止まって静止していること。
> 起動していない場合はスタートメニューから「Docker Desktop」を起動し、クジラが静止するまで待つ（1〜2分かかる）。
...
### Step 10：Djangoプロジェクトを初期化する

`docker-compose.yml` は `python manage.py runserver` を起動するが、まだ `manage.py` が存在しない。コンテナを使って Django プロジェクトを生成する。

```bash
# まずイメージだけビルドする（runserver はまだ実行しない）
docker compose build

# コンテナの中で django-admin を実行してプロジェクトを作成する
...
│   ├── urls.py
│   └── wsgi.py
├── manage.py          ← これがないと runserver が動かない
├── .env
├── .env.example
├── .gitignore
├── Dockerfile
...
### Step 12：PR を作成してマージする

```bash
git add Dockerfile docker-compose.yml requirements.txt .env.example .gitignore manage.py config/
...
| `ports are not available: exposing port TCP 0.0.0.0:8000` | Windows の Hyper-V がポート 8000 を予約済み | **PowerShell を管理者で開いて** `net stop winnat` → `net start winnat` を実行し、再度 `docker compose up --build` を試す。それでも解消しない場合は下記「ポート変更手順」を参照 |

---

## 5-2. ポート変更手順（Windows でポート 8000 が使えない場合）

`net stop winnat` / `net start winnat` を試してもエラーが続く場合は、ポートを 8080 に変更する。

**docker-compose.yml の変更箇所：**

```yaml
    ports:
      - "8080:8000"   # ← 左側（ホスト側）を 8080 に変更
```

**manage.py runserver のコマンドはそのまま**（コンテナ内は 8000 のまま）。

変更後は `http://localhost:8080` でアクセスする。
